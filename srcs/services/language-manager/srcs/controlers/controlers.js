...
export async function get_user_lang_route(request, reply) {
    ...
    try {
        ...
        console.log("userId in get-lang:", userId);
        if (!userId) return reply.code(400).send({ success: false, message: "Missing user_id" });

        let userLang = await app.db.get("SELECT lang FROM user_lang WHERE user_id = ?", [userId]);
        if (!userLang) {
          await app.db.run("INSERT INTO user_lang (user_id, lang) VALUES (?, ?)", [userId, 'en']);
          userLang = { lang: 'en' };
        }
...
export async function set_user_lang_route(request, reply) {
  try {
    ...
    if (!user_id || !lang)
      return reply.code(400).send({ success: false, message: "Missing user_id or lang" });
