...
var AIPage = /** @class */ (function () {
    ...
    ///////////// DESIGN & RENDERING /////////////
    AIPage.prototype.render = function (user) {
        var _this = this;
        if (user !== undefined) {
            this.user = user;
        }
        ...
        var player2Label = this.uiManager.createElement('div', 'text-lg opacity-60', 'AI BOT');
        ...
        var startTitle = this.uiManager.createElement('div', 'text-3xl mb-6 text-[#ff1493]', 'CHOOSE DIFFICULTY');
        var startButtonEasy = this.uiManager.createButton('EASY', 'retro-button bg-[#ff1493] text-black px-8 py-3 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-[#ff1493] transition-all duration-200', function () { return _this.selectDifficulty('easy'); });
        var startButtonMedium = this.uiManager.createButton('MEDIUM', 'retro-button bg-[#ff1493] text-black px-8 py-3 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-[#ff1493] transition-all duration-200', function () { return _this.selectDifficulty('medium'); });
        var startButtonHard = this.uiManager.createButton('HARD', 'retro-button bg-[#ff1493] text-black px-8 py-3 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-[#ff1493] transition-all duration-200', function () { return _this.selectDifficulty('hard'); });
    ...
    /////////// GAME INITIALIZATION ////////////
    //////////////////////////////////////////////
    AIPage.prototype.selectDifficulty = function (difficulty) {
        ...
        GameManager_js_1.GameManager.requestGameID("ai", { difficulty: difficulty });
    };
    AIPage.prototype.requestNewGame = function () {
    ...
    AIPage.prototype.setReady = function () {
        if (!this.gameManager)
            return;
    ...
    AIPage.prototype.setupGame = function (data) {
    ...
    AIPage.prototype.setupGameListeners = function () {
        var _this = this;
        if (!this.gameManager)
            return;
    ...
    ///////////// UPDATES ////////////////////
    //////////////////////////////////////////
    AIPage.prototype.updateScore = function (player1Score, player2Score) {
    ...
    AIPage.prototype.updateGameState = function (gameState) {
        var _a, _b;
        var startOverlay = document.querySelector('[data-overlay="start-game"]');
        ...
        // Screen at the start of the game
        if (!((_b = this.gameManager) === null || _b === void 0 ? void 0 : _b.getHasStarted())) {
            if (startOverlay)
                startOverlay.classList.remove('hidden');
            if (pauseOverlay)
                pauseOverlay.classList.add('hidden');
    ...
    ///////////// GAME FUNCTIONS ////////////
    //////////////////////////////////////////
    AIPage.prototype.pauseGame = function () {
        if (!this.gameManager)
            return;
    ...
    AIPage.prototype.backToMenu = function () {
        if (this.gameManager) {
            this.gameManager.destroy();
            this.gameManager = null;
        }
    ...
    AIPage.prototype.resetGame = function () {
        if (!this.gameManager)
            return;
