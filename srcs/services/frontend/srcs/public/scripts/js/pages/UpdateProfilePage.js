...
export class UpdateProfilePage {
    ...
    buildProfilePage() {
        ...
        const card = this.uiManager.createElement('div', 'bg-black/40 backdrop-blur-sm border-2 border-[#ff1493] rounded-lg p-8 shadow-[0_0_30px_#ff1493] w-full max-w-md flex flex-col items-center gap-8 mt-16');
        // Avatar Section
        ...
        const oauth42Badge = this.uiManager.createElement('p', 'text-sm text-[#00ffff]/90 font-medium mt-1');
        ...
        if (!isOAuth42) {
            const usernameResult = createField(this.t('username'), 'text', this.t('usernamePlaceholder'), async (value) => {
                ...
                if (!res.ok) {
                    ...
                    if (usernameResult.errorDiv)
                        usernameResult.errorDiv.textContent = msg;
                    return;
                }
                if (this.user)
        ...
        let emailField = null;
        if (!isOAuth42) {
            const emailResult = createField(this.t('email'), 'email', this.t('emailPlaceholder'), async (value) => {
                if (!value) {
                    console.info('[UpdateProfile]', this.t('noEmailEntered'));
                    if (emailResult.errorDiv)
                        emailResult.errorDiv.textContent = this.t('noEmailEntered');
                    return;
                }
                ...
                try {
                    ...
                    if (!res.ok) {
                        ...
                        console.info('[UpdateProfile]', msg);
                        if (emailResult.errorDiv)
                            emailResult.errorDiv.textContent = msg;
                        return;
                    }
                    ...
                    if (!data1.success) {
                        ...
                        if (emailResult.errorDiv)
                            emailResult.errorDiv.textContent = data1.message || this.t('failedUpdateEmail');
                        return;
                    }
        ...
        const { fieldContainer: passwordField, errorDiv: passwordErrorDiv } = createField(this.t('password'), 'password', this.t('enter_password'), async (value, confirmValue) => {
            ...
            if (value !== confirmValue) {
                ...
                if (passwordErrorDiv)
                    passwordErrorDiv.textContent = "Passwords don't match";
                return;
            }
            ...
            if (!res.ok) {
                ...
                console.info('[UpdateProfile]', msg);
                if (passwordErrorDiv)
                    passwordErrorDiv.textContent = msg;
                return;
            }
            this.render();
        }, true);
    ...
    renderAvatarSelector() {
        ...
        const card = this.uiManager.createElement('div', 
        ...
        const inner = this.uiManager.createElement('div', 'w-full max-w-md flex flex-col items-center gap-8');
        const title = this.uiManager.createElement('h1', 
