...
export class SocialManager {
    constructor(uiManager, routerManager, wsManager, languageManager, currentUser, getCurrentSelectedFriendId, onFriendSelect, onNewMessage, onGameInvite) {
        ...
        this.pendingGameInvites = new Map();
    ...
    restorePendingGameInvites(container) {
        if (!container)
            return;
    ...
    async sendTypingSignal(friendId) {
    ...
    addGameInviteNotification(data) {
        const container = document.getElementById('notifications-container');
        if (!container)
            return;
        this.addGameInviteNotificationToContainer(data, container);
    }
    addGameInviteNotificationToContainer(data, container) {
    ...
    removeGameInviteNotification(inviteId) {
...
SocialManager.DEBUG_INVITE = false;
