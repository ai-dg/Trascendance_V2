...
export function getConnectedOptions(text: Translations) {
 ...
 export async function showUpdateProfile(text:Translations) {
    ...
    getElement<HTMLButtonElement>('changeUsernameBtn')?.addEventListener('click', () => {
	...
	getElement<HTMLButtonElement>('changeEmailBtn')?.addEventListener('click', () => {
	...
	getElement<HTMLButtonElement>('changePasswordBtn')?.addEventListener('click', () => {
		const newPass = (getElement<HTMLInputElement>('passwordInput')?.value ?? '').trim();
		const confirmPass = (getElement<HTMLInputElement>('confirmPasswordInput')?.value ?? '').trim();
