...
export class GuestPage {
  ...
  public render(): void {
    ...
    this.selectAvatar('avatar1', avatarContainer.querySelector('[data-avatar="avatar1"]') as HTMLElement);
  }

  private selectAvatar(avatarId: string, element: HTMLElement): void {
    // Remove selection from all avatars
    const allAvatars = document.querySelectorAll('[data-avatar]');
    allAvatars.forEach(avatar => {
      const circle = avatar.querySelector('div') as HTMLElement;
      circle.classList.remove('border-[#ff1493]', 'ring-4', 'ring-[#ff1493]/50', 'scale-110');
      circle.classList.add('border-transparent');
    });
    
    // Add selection to clicked avatar
    const circle = element.querySelector('div') as HTMLElement;
