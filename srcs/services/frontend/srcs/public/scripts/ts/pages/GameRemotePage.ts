...
export class RemotePage {
  ...
  constructor(uiManager: UIManager, languageManager: LanguageManager, onBack: () => void, user?: User | null) {
  ...
  public render(user?: User | null, routeData?: RouteData): void {
    ...
    const startButtonRandom = this.uiManager.createButton(
        this.t('playAgainstRandom') || 'PLAY AGAINST RANDOM PLAYER',
    ...
    const pauseButton = this.uiManager.createButton(
      this.t('pauseResume') || 'PAUSE / RESUME',
  ...
  private showReconnectionOption(data: any): void {
    ...
    if (startOverlay) {
      ...
      const message = this.uiManager.createElement('div', 'text-lg mb-6 text-white',
        data.message || this.t('reconnectPrompt') || 'You have an ongoing game. Would you like to reconnect?'
      );
      ...
      const newGameButton = this.uiManager.createButton(
        this.t('startNewGame') || 'START NEW GAME',
    ...
    GameManager.reconnectToGame(gameUUID, (result) => {
      if (result.success) {
        ...
        if (startOverlay) {
          ...
          const message = this.uiManager.createElement('div', 'text-lg mb-6 text-white',
            this.t('clickReadyToResume') || 'Both players click READY to resume the game.'
          );
        ...
        if (startOverlay) {
          ...
          const message = this.uiManager.createElement('div', 'text-lg mb-6 text-white',
            result.message || this.t('couldNotReconnect') || 'Could not reconnect to the game.'
          );
          const backButton = this.uiManager.createButton(
            this.t('backToLobby') || 'BACK TO LOBBY',
            'retro-button bg-[#ff1493] text-black px-8 py-3 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-[#ff1493] transition-all duration-200',
            () => this.requestNewGame()
          );
          content.appendChild(title);
    ...
    if (startOverlay) {
      ...
      if (data.waitingForReconnection) {
        const waitMessage = this.uiManager.createElement('div', 'text-sm mb-4 text-[#9d4edd]',
          this.t('waitingReconnectTimeout') || 'Waiting for opponent to reconnect... (2 minute timeout)'
        );
        content.appendChild(waitMessage);
      }

      const leaveButton = this.uiManager.createButton(
        this.t('leaveGame') || 'LEAVE GAME',
        'retro-button bg-[#ff1493] text-black px-8 py-3 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-[#ff1493] transition-all duration-200',
        () => this.backToLobby()
      );
  ...
  private handleReconnectionTimeout(data: any): void {
    ...
    if (startOverlay) {
      ...
      const message = this.uiManager.createElement('div', 'text-lg mb-6 text-white',
        data.message || this.t('opponentTimeoutMsg') || 'Opponent did not reconnect in time. The game has ended.'
      );
      const backButton = this.uiManager.createButton(
        this.t('backToLobby') || 'BACK TO LOBBY',
        'retro-button bg-[#ff1493] text-black px-8 py-3 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-[#ff1493] transition-all duration-200',
  ...
  private handleOpponentAbandoned(data: any): void {
    ...
    if (startOverlay) {
      ...
      const message = this.uiManager.createElement('div', 'text-lg mb-6 text-white',
        data.message || this.t('opponentStartedNewGameMsg') || 'Your opponent has started a new game. This game has been ended.'
      );
      const backButton = this.uiManager.createButton(
        this.t('backToLobby') || 'BACK TO LOBBY',
        'retro-button bg-[#ff1493] text-black px-8 py-3 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-[#ff1493] transition-all duration-200',
...
//////////////////////////////////////////////
  ...
  private playAgainstFriend(): void
  {
    ...
    const cancelButton = this.uiManager.createButton(
      this.t('cancel') || 'CANCEL',
      'retro-button bg-transparent text-[#ff1493] px-6 py-2 rounded border-2 border-[#ff1493] hover:bg-transparent hover:text-black transition-all duration-200',
      () => {
        this.removeWaitingScreen();
        this.isSearchingOpponent = false;
        document.querySelector<HTMLElement>('[data-overlay="start-game"]')?.classList.remove('hidden');
