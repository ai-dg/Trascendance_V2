...
export class RemotePage {
	...
	public render(user?: User | null, data?: any): void {
		if (user !== undefined) {
			this.user = user;
		}

		// Private game join flow
		if (data && data.mode === 'private' && data.gameUUID) {
			this.joinPrivateGameFlow(data.gameUUID);
			return;
		}

		// ...existing code for avatars, overlays, etc...
		...
		// ...existing code for overlays, controls, etc...
	...
	// --- Private Game Join Flow ---
	private joinPrivateGameFlow(gameUUID: string): void {
		if (!this.canvas) {
			// Render base UI to get canvas
		...
		// Join the private game on server
		if (gameSocket) {
			gameSocket.emit('join-private-game', { gameUUID });
			// Wait for joined-private-game event
			gameSocket.once('joined-private-game', (data) => {
				this.gameManager?.setPlayerNumber(data.playerNumber);
				// Will receive opponent-found when both players join
			});
		}
	}
