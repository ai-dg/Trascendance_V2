...
import { setRemoteAvatar, User } from "./User";
import { get_user_stats } from "./stats";


export const defaultAvatarBackgroundColors: string[] = [
    "#2C3E50", // Bleu minuit
    "#8E44AD", // Prune
    "#3B3B98", // Indigo foncé
    "#27AE60", // Vert sapin
...
export const avatar_map : {[key: string]: Blob } = {}
...
export class MouseMove
{
    ...
    static getDistance(newpos:Vector):Vector
    {
        let distance:Vector = { 
            x: MouseMove.#offsetX,
            y: MouseMove.#offsetY
    ...
    static setDistance(newpos:Vector):void
    {
...
export function moveElement(event: MouseEvent):void
{
    let x:number = event.clientX; // position x de la souris
    let y:number = event.clientY; // position y de la souris
    let move = MouseMove.getDistance({ x:x, y:y }) // déplacement X et Y par rapport a la position précédente...
    let child = document.getElementById("crop_box") as HTMLCanvasElement
    let imgctx: CanvasRenderingContext2D = child.getContext("2d") as CanvasRenderingContext2D;    
    let image = document.getElementById("loadedimage") as HTMLImageElement;
    ...
    imgctx.drawImage(
    image,
    sourceX,          // position x ajustée dans l'image source
...
export function selectImage(event:Event):void{
    const load_avatar:HTMLInputElement = document.getElementById("avatar") as HTMLInputElement;
    const child = document.getElementById("crop_box") as HTMLCanvasElement
    const image = document.querySelector(".avatar-view") as HTMLElement
    const avatar_crop_wrapper = document.querySelector(".avatarCropWrapper") as HTMLElement
    const crop_btn = document.querySelector("#crop-button") as HTMLElement
    const change_avatar_btn = document.querySelector('#change-avatar-btn') as HTMLElement;  
    let dataURL = child.toDataURL();
    const preview: HTMLElement = document.querySelector(".avatar-preview") as HTMLElement;
...
export async function loadAvatar(username: string, element:HTMLElement, format: Avatar_size | null) {
...
export async function getAvatarFromUser(username: string, element:HTMLElement, format: Avatar_size | null = null)
{
...
export function getAvatar(blob:Blob |null, format:Avatar_size | null, username:string):HTMLElement
{
...
export function getDefaultAvatar(username:string | null = null, format: Avatar_size | null): HTMLElement
{
    const avatar = document.createElement("div")
    avatar.classList.add("avatar")
    if (!username)
...
export async function setDefaultAvatar(username:string | null)
{
    if (!username)
        return null;
    ...
    canvas.height = 128
    canvas.width = 128
    let imgctx: CanvasRenderingContext2D = canvas.getContext("2d") as CanvasRenderingContext2D;
    let color_index = Math.floor(Math.random() * defaultAvatarBackgroundColors.length)
    let color:string = defaultAvatarBackgroundColors[color_index]
...
export function reloadAvatar(username:string){
   
    let elements =  Array.from(document.getElementsByClassName(`${username}_avatar`) as HTMLCollectionOf<HTMLImageElement>)
    const formdata = new FormData();
    formdata.append("username", username)
    fetch(getUrl("accounts/avatar/?username=" + username),
    {
