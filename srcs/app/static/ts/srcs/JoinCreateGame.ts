 import { socket_game } from "./RegisterSockets";
 import { User } from "./User";
 import { Game } from "./Game";
 import { initSocket } from "./Sockets";
 import { default_map, invited_map, invited_inverted_map } from "./key_map";
 import { register_game, USER_ACTION } from "./handlers";
import { close_windows, notification_message } from "./uitools";
import { message_notification } from "./Chat";
import { display_game_count, display_game_message, display_game_score, clear_all_canvas } from "./GameNotifications";
import { game_scene } from "./Scene";
import { update_game_alert } from "./Chat";
import { updatePills } from "../app";
...
const ready_btn = document.querySelector(".ready") as HTMLElement
const resume_btn = document.querySelector(".resume") as HTMLElement
const active_games: Record<string, WebSocket> = {};

export const games_lobby = document.querySelector(".games-lobby")
export let game_socket_is_ready = false
export let game_socket_outside: WebSocket | null = null
export let key_map: any = default_map

let countdown_id: number |null = null;
let timer = 5;
...
export function sendMoveCommand(action: string) {
...
export function keydown_handler(event:KeyboardEvent){
...
export function keyup_handler(event:KeyboardEvent){
    if (`${event.key}` in key_map && key_map[`${event.key}`] !== undefined) {
...
export function update_tournament_lobby(games:any, add = false){
    const join_game_list : HTMLElement = document.getElementById("tournament_list") as HTMLElement;
    if (!add)
...
export function update_game_lobby(games:any, add = false){
...
export function update_lobbies(data:any)
{
...
export function register_tournament(tournament:any)
{
...
function getGameElement(game: any)
{
    let el : HTMLElement = document.getElementById(game.game_uid) as HTMLElement;
    if (!el)
    {
        ...
        let owner: HTMLElement = document.createElement('h6');
...
export function update_game_display(game:any){

    let el : HTMLElement = getTournamentElement(game[0]);

    el.remove();
}

export function remove_game(game_uid:string)
{
...
export function update_tournament_display(game:any){
    game = game[0]
    console.log(game)
    let el : HTMLElement = getTournamentElement(game);
}


async function send_ready_state(socket:WebSocket, el:HTMLElement){
    socket.send(JSON.stringify({command:"start"}))
}



export function display_action_button(socket:WebSocket, btn: HTMLElement){

    const handleReadyClick = async (e:Event) => {
        await send_ready_state(socket, btn as HTMLElement);
...
export function launch(params:any, game_socket_id:WebSocket)
{
    if (game_socket_id)
    {
...
export async function play(params:any)
{
    let game_socket_id : WebSocket | null = null;
    ...
    else
    {
        ...
        game_socket_id.onmessage = (event: MessageEvent) => run(event, game_socket_id!);
        game_socket_id.onerror = (event: Event) => console.log("❌ WebSocket error:", event);
...
export async function run(event:MessageEvent, game_socket_id:WebSocket)
{
    ...
    else if (data_game.status && data_game.status =="running")
    {
        ...
        lastUpdate = now;
        if ((window as any).gameInstance) {
            (window as any).gameInstance.start();
            (window as any).gameInstance.updateGame(data_game);
        } else {
            console.log("🎮 Creating a new game instance!");
            (window as any).gameInstance = new Game(data_game);
        }
    }
...
function format_players(players : []): string
{
    let str : string = ""
...
function getTournamentElement(game: any):HTMLElement
{
    let el : HTMLElement = document.getElementById(game.tournament_uid) as HTMLElement
    if (!el)
    {
        el = document.createElement("div")
        el.classList.add("container", "d-flex");
    ...
    }else
    {
        if (game.players)
        {
            let slots = el.querySelector(".slots") as HTMLElement
            slots.innerHTML = "Players: " + game.players.length + " / " + game.expected + " - " +  format_players(game.players);
            
            let join = el.querySelector(".gameinfo")!.querySelector(".join") as HTMLElement
