...
import { Numbers, ScoreData } from "./Numbers";
import { game_scene, score_scene } from "./Scene";
...
export function wait_and_clear(time:number)
{
    current_timeout = setTimeout(clear_all_canvas, time)
}

export function display_game_message(message:string)
{
...
export function display_game_count(message:string, conf= false)
{
    game_scene.clear()
    cancel_timeout()
    let canvas = document.getElementById("gameCanvas") as HTMLCanvasElement;
...
function get_game_stats(stats:any)
{
    const games = stats.games;
    const players:string [] = stats.players
    let map = new Map()
    ...
    return rank.map(([name, _]) => name);
}


export function display_game_stats(stats:any)
{
    game_scene.clear()
    score_scene.clear()
    cancel_timeout()
...
export function display_game_score(data:any)
{
    cancel_timeout()
    console.log(data)
    let param: ScoreData
