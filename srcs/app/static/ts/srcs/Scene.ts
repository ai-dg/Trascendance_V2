...
export class Scene {
    ...
    constructor(data: { scene: { canvas_id: string; width?: number; height?: number } }) {
        this.canvas = document.getElementById(data.scene.canvas_id) as HTMLCanvasElement | null;
    ...
    draw(objects: { draw: (ctx: CanvasRenderingContext2D) => void }[]) {
        if (!this.ctx) return;
